...
bool pawnsCantMove(Player* player, int x) {
	if (!(((player->pawns[0]->inBase == 0 && player->pawns[0]->inFinal == 0) && (goingToFinal(player, 0, x) || nextNotFull(player, 0, x))) || ((player->pawns[1]->inBase == 0 && player->pawns[1]->inFinal == 0) && (goingToFinal(player, 1, x) || nextNotFull(player, 1, x))) || ((player->pawns[2]->inBase == 0 && player->pawns[2]->inFinal == 0) && (goingToFinal(player, 2, x) || nextNotFull(player, 2, x))) || ((player->pawns[3]->inBase == 0 && player->pawns[3]->inFinal == 0) && (goingToFinal(player, 3, x) || nextNotFull(player, 3, x)))))
		return 1; else return 0;
}
...
void turn(Player* player)
{
	...
	_getch();
	map();
	if ((x != 6) && (pawnsCantMove(player, x)))
	{cout << "aaaa";
	_getch(); }//    0 PAWNS ON BOARD	|	dice 1-5
