...
void start()
{
	...
	red.pawns[0]->position = 8;
	red.pawns[1]->inBase = 0;
	red.pawns[1]->position = 8;
	red.pawns[2]->inBase = 0;
	red.pawns[2]->position = 7;
	red.pawns[3]->inBase = 0;
	red.pawns[3]->position = 7;
	fields[8] = '8';
	fields[7] = '8';
	*/
	map();
	cout << "Press any key to start...";
	while (!((red.ended == 1 && blue.ended == 1 && green.ended == 1) || (red.ended == 1 && blue.ended == 1 && yellow.ended == 1) || (red.ended == 1 && green.ended == 1 && yellow.ended == 1) || (blue.ended == 1 && green.ended == 1 && yellow.ended == 1))) {
		if (red.ended == 0)
		{
			map();
			turn(&red);
